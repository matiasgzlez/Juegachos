...
Registered in `GAME_SCORING` (see root `CLAUDE.md`) with `direction: "lower"` and `variants: ["3", "4", "5"]`, so each board size has its own independent leaderboard. The board is **ranked by time**, with the move count shown beside it. Since the leaderboard stores a single numeric `score`, `handleVictory()` submits an encoded value from `encodeSlidingScore(elapsedTime, moves)` (in `src/shared/scoring.ts`): `centiseconds * BASE + moves`, so sorting ascending ("lower") orders by time and uses moves as the tiebreak. The game's `format` (`formatSlidingScore`) decodes it back to `M:SS.cc - N mov`. Both time and moves are still tracked per size in localStorage as personal bests.

## Room mode (multiplayer)

Wired to the shared party mode: the constructor calls `initRoomMode("sliding-puzzle", { getScore: () => encodeSlidingScore(this.elapsedTime, this.moves) })` (see root `CLAUDE.md`, "Salas (multiplayer rooms)"). With `?room=` in the URL the board size is fixed to 4x4 (`ROOM_VARIANTS["sliding-puzzle"]`) and the size selector is hidden so everyone solves the same puzzle; `handleVictory()` reports the same encoded time+moves score to the room instead of the global ranking (so room placement is by time too) and the Enter-to-retry on victory is blocked (one run per round). The timeout partial is the current encoded score with `finished=false` — since this game is `direction: "lower"`, `points.ts` treats unsolved partials as non-comparable and ranks them behind every solved board.

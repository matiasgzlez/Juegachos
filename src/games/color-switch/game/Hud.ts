...
export class Hud {
  ...
  constructor(container: HTMLElement) {
    ...
    this.hintEl.textContent = "espacio / clic / toca para impulsar";
  ...
  showStart(): void {
    this.titleEl.innerHTML = spectralTitle("PRISMA");
    this.subtitleEl.textContent = "Cruzá cada anillo solo por el arco de tu color.";
  ...
  showGameOver(score: number, best: number): void {
    this.titleEl.innerHTML = spectralTitle("SE ROMPIÓ");
...
/** Envuelve el texto para pintarlo con el degradé espectral (via CSS). */
function spectralTitle(text: string): string {
  return `<span class="overlay__wordmark">${text}</span>`;
}

...
- `style.css` — Estilos de la interfaz. Escena en **perspectiva de 3ra persona**: el tablero (`.board::before`) dibuja una mesa que se aleja con cuadricula y borde neon (plano inclinado con `perspective` + `rotateX`) sobre la que se apoyan los vasos. Cada vaso tiene forma conica real (cuerpo `.cup-body` con `clip-path` trapezoidal + tapa curva y brillo con pseudo-elementos, boca eliptica de neon `.cup-rim` apoyada en la mesa, y sombra de contacto eliptica en `.cup::after`). El color/glow de estado (hover, seleccionado, correcto/incorrecto) se controla con variables CSS por vaso (`--cup-edge` / `--cup-face` / `--cup-glow`) en lugar de reescribir `border`/`box-shadow`, y el glow usa `filter: drop-shadow` (que sigue el `clip-path`, a diferencia de `box-shadow` que se recorta). La moneda dorada tiene relieve (bordes, brillos internos) y gira con efecto de volteo. Incluye las animaciones de curva 3D para los swaps y la cuenta regresiva.

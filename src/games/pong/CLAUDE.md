...
- **The server is the ball authority.** `server/src/games/pong.ts` (`PongSim`) pairs the room's `roster` `(0,1),(2,3),...` into one **match per pair** (odd one out plays vs a server AI), runs the ball physics / collisions / speed ramp / scoring for every match on a ~60 fps interval (`TICK_MS`), and broadcasts `pg:state` to each seated player (their own `side` = `"p1"` left / `"p2"` right, both scores, both paddle Ys, the ball, and `vsAi`). **First to `SCORE_LIMIT` (3) goals** ends that match; the constant is mirrored client-side (used only by the vs-AI fallback). This replaced the old P1-authority Supabase-broadcast model (`PongChannel`, deleted), which was jittery for the non-host.
  - **Sub-stepping against tunneling.** Within a tick the ball is integrated in sub-steps of at most `SUBSTEP_MAX_PX` (5 px), checking paddle/goal collision after each, so a fast ball (up to `BALL_SPEED_MAX`) can never jump over a paddle without bouncing.

- **The client owns only its paddle; everything else is entity-interpolated.** Each frame the local paddle follows local input immediately (mouse / arrows / W-S, combined — every player is on their own device controlling one paddle) and its Y is sent to the server frequently (`BROADCAST_INTERVAL`, 50 Hz, `pg:paddle`). The **ball and the opponent paddle are rendered by interpolating the server's snapshots on a fixed delay** (`renderFromSnaps`, `BALL_INTERP_DELAY` = 80 ms) — the standard Source-style netcode. Each `pg:state` while playing is pushed into a timestamped ring buffer (`snaps`, `pushSnap`); each frame the ball + opponent paddle are read at `renderTime = now - BALL_INTERP_DELAY` from the two bracketing samples (both on the **same** timeline so the bounce looks consistent). This deliberately replaced the earlier client-side ball *prediction + reconciliation*, which overshot the paddle (the client had no authoritative paddle to predict the bounce) and then yanked the ball back — the visible stutter/rubber-band. A snapshot whose ball jumps more than `BALL_SNAP_DIST` (goal reset / relaunch) resets the buffer so the teleport isn't interpolated. Hit/score **sounds** are inferred from the snapshot diff (`ball.hits` / score change in `playSnapSounds`) since the server sends no per-bounce event. Trade-off: the ball you see trails the server by ~`BALL_INTERP_DELAY` + network latency; lower it for less delay at the cost of jitter tolerance.

- **Side comes from the server, not computed.** `initRoomMode` loads `room.players()` async (`boot()`), so the constructor can't know the pairing. The client just connects on `beginCountdown()` (`connectServer`) announcing `{code, nickname, roster}` and learns its `side` from the first `pg:state`; it never derives pairing itself, so there's no client/server drift. `hintFixed` sets the "sos J1/J2" hint once from that first snapshot. **Manual Enter is ignored in server mode** (`onAction`): the round is started only by `onStart` (once RoomMode reaches `playing`, with `room.players()` fully loaded), so the client never connects with a partial roster that would make the server start someone alone vs AI.
...
- **Scoring.** Each player reports their OWN goals via `this.room.reportScore(this.score)` on `pg:state.phase === "over"` (higher = better, the default board). Room scores never go to the global leaderboard. The HUD shows two symmetric markers in room mode (`Hud.showScoreRoom` adds `hud__score--room`: left = J1/`p1`, right = J2/`p2`, positioned at 38%/62% around the center net); solo mode keeps the single centered score.

- **Degradation.** In room mode **without** `VITE_GAME_SERVER_URL`, Pong falls back to a **local vs-AI** match per player (`updateUnpaired` + `checkCollisionsRoom`, first to `SCORE_LIMIT`) that still reports a score — so the room never gets stuck. The same fallback triggers **at runtime** if the socket can't reach the server: `PongSocket` forwards `connect_error` (bad URL / CORS / the `/pong` namespace missing because the server wasn't redeployed), and after `SERVER_ERROR_GRACE_MS` (~3s) with no `pg:state` the client calls `fallbackToLocalAi` instead of leaving the ball frozen at center. Note this means "everyone vs AI" is the symptom of a mis-deployed/mis-configured server — real PvP requires the server reachable (redeployed with the namespace, `VITE_GAME_SERVER_URL` set on Vercel, and the Vercel origin in the server's `ALLOWED_ORIGINS`). Without `?room=` at all it's the normal solo endless mode.
...
- **Architecture files:**
  ...
  - `game/Game.ts` — routing in `update()`: `updateServer` (room + server), `updateUnpaired` (room, no server = local AI), `updateSolo` (no room). `onServerState` maps the snapshot into local scores + `pushSnap`; `renderFromSnaps` places the ball/opponent paddle by interpolation each frame.

Tuning that must stay in sync with the server (`server/src/games/pong.ts`) because the server duplicates the physics constants: `constants.ts` (view size, paddle/ball geometry, speeds, ramp) and `SCORE_LIMIT` (3). If you change ball/paddle tuning, change both files. Netcode feel knobs: client `BALL_INTERP_DELAY` / `BROADCAST_INTERVAL`; server `TICK_MS` / `SUBSTEP_MAX_PX` / `PREROLL_MS`.
